...
const heightValues = Array.from({ length: HEIGHT_MAX - HEIGHT_MIN + 1 }, (_, idx) => HEIGHT_MIN + idx);
...
export default function OnboardingScreen() {
  ...
  const heightListRef = useRef<FlatList<number>>(null);

  const canContinue = useMemo(() => {
    if (step === 1) return true;
    if (step === 2) return age > 0;
    if (step === 3) return height > 0;
    if (step === 4) return !!gender;
    if (step === 5) return !!bodyType;
    if (step === 6) return !!skinTone;
    if (step === 7) return stylePreferences.length === 3;
    return false;
  }, [step, age, height, gender, bodyType, skinTone, stylePreferences.length]);
  ...
  const onAgeViewableItemsChanged = useRef(({ viewableItems }: { viewableItems: ViewToken[] }) => {
    const centered = viewableItems.find((item) => item.isViewable && item.item != null);
    if (centered?.item) setAge(centered.item as number);
  }).current;

  const onHeightViewableItemsChanged = useRef(({ viewableItems }: { viewableItems: ViewToken[] }) => {
    const centered = viewableItems.find((item) => item.isViewable && item.item != null);
    if (centered?.item) setHeight(centered.item as number);
  }).current;
      ...
      <View className="flex-1 px-6 pb-6 pt-2">
        ...
        {step === 1 && (
          <View className="flex-1 items-center justify-center gap-6">
            ...
            <Text className="text-base text-center text-gray-500">Let&apos;s personalize your experience in a few quick steps.</Text>
          </View>
        )}
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <View className="items-center justify-center" style={{ width: AGE_ITEM_WIDTH }}>
                      <Text className={`font-semibold ${selected ? "text-5xl text-white" : "text-4xl text-[#5F52A2]"}`}>{item}</Text>
                    </View>
                  );
                }}
              />
        ...
        {step === 3 && (
          <View className="flex-1 items-center">
            <Text className="text-center text-5xl font-semibold tracking-tight text-[#1D1A27]">What is your height?</Text>
            ...
            <View className="relative mt-8 h-[300px] w-[116px] overflow-hidden rounded-2xl bg-[#A89AF4] py-2">
              <FlatList
                ...
                onViewableItemsChanged={onHeightViewableItemsChanged}
                renderItem={({ item }) => {
                  const selected = item === height;
                  return (
                    <View className="h-[52px] items-center justify-center">
                      <Text className={`${selected ? "text-4xl font-bold text-black" : "text-3xl font-semibold text-[#6E6A79]"}`}>{item}</Text>
                    </View>
                  );
                }}
              />
            ...
            <Text className="absolute right-[70px] top-[270px] text-2xl text-[#D4DD56]">◀</Text>
          </View>
        )}
        ...
        {step === 4 && (
          <View className="flex-1">
            ...
            <View className="mt-16 items-center gap-8">
              {[{ label: "Male", icon: "♂", bg: "#1E1A27", iconColor: "#FFFFFF" }, { label: "Female", icon: "♀", bg: "#DCE754", iconColor: "#1E1A27" }, { label: "Other", icon: "⚥", bg: "#5E59E6", iconColor: "#FFFFFF" }].map((option) => {
                ...
                return (
                  <Pressable key={option.label} onPress={() => setGender(option.label as "Male" | "Female" | "Other")} className="items-center">
                    <View className={`h-28 w-28 items-center justify-center rounded-full border-4 ${selected ? "border-[#1D1A27]" : "border-transparent"}`} style={{ backgroundColor: option.bg }}>
                      <Text className="text-6xl font-semibold" style={{ color: option.iconColor }}>{option.icon}</Text>
                    </View>
        ...
        {step === 5 && (<View className="flex-1 gap-5"><Text className="text-3xl font-bold text-gray-900">Select your body type</Text><View className="flex-row flex-wrap gap-3">{bodyTypes.map((type, idx) => (<Pressable key={type} onPress={() => setBodyType(type)} className={`w-[48%] rounded-2xl border p-3 ${bodyType === type ? "border-blue-600" : "border-gray-300"}`}><View className="h-24 items-center justify-center rounded-xl bg-gray-100"><Text className="text-4xl">{["🧍", "🏃", "🧑", "💃", "🕺"][idx]}</Text></View><Text className="mt-2 text-center text-base text-gray-800">{type}</Text></Pressable>))}</View></View>)}
        {step === 6 && (<View className="flex-1 gap-5"><Text className="text-3xl font-bold text-gray-900">Select your skin tone</Text><View className="flex-row flex-wrap gap-3">{skinTones.map((tone) => (<Pressable key={tone} onPress={() => setSkinTone(tone)} className={`h-16 w-16 rounded-full border-2 ${skinTone === tone ? "border-blue-600" : "border-transparent"}`} style={{ backgroundColor: tone }} />))}</View></View>)}
        {step === 7 && (<View className="flex-1 gap-5"><Text className="text-3xl font-bold text-gray-900">Style preferences</Text><Text className="text-sm text-gray-500">Choose exactly 3 styles</Text><View className="flex-row flex-wrap gap-3">{styles.map((style) => {const selected = stylePreferences.includes(style);return (<Pressable key={style} onPress={() => toggleStyle(style)} className={`rounded-full border px-4 py-3 ${selected ? "border-blue-600 bg-blue-50" : "border-gray-300"}`}><Text className="text-base text-gray-800">{style}</Text></Pressable>);})}</View></View>)}
        ...
        {step < 8 && (
          <Pressable onPress={onContinue} disabled={!canContinue} className={`mt-auto items-center rounded-2xl py-4 ${canContinue ? "bg-[#1A1827]" : "bg-gray-300"}`}>
